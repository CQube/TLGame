...
{
	mainHero = new MainHero(device, "../media/sydney.md2", "../media/sydney.bmp", vector3df(180, 200, 0));
	AnimNode *node_sydney = mainHero->getAnimNode();
	node_sydney->setScale(core::vector3df(1.5f));
	node_sydney->setMD2Animation(scene::EMAT_STAND);

	mainHero->addTrSelector(selector);
...
void Level::run()
{
	...
	camera->setFocusMesh(mainHero->getAnimNode());

        device->getCursorControl()->setVisible(false);
	
	while(device->run())
	{
		mainHero->Move(receiver);
		
		if(receiver->IsKeyDown(irr::KEY_KEY_Q)){
			vector3df pos_node = mainHero->getAnimNode()->getPosition();
		}
